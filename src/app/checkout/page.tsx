...
const DUMMY_CHECKOUT_PRODUCT: CheckoutProduct | null = null;
...
export default function CheckoutPage() {
    ...
    return (
      <div className="flex flex-col min-h-screen bg-surface-tint">
        ...
        <main className="flex-1 flex flex-col items-center justify-center p-6 text-center">
          ...
          <p className="text-[14px] text-text-sub mt-2 mb-8">Pilih item dari keranjang Anda untuk melanjutkan checkout.</p>
  ...
  const subtotal = DUMMY_CHECKOUT_PRODUCT.price;
  const shippingFee = selectedShipping.price;
  const appFee = 2500;
  const discount = voucherCode === "POKEMON10" ? 185000 : 0;
  const total = subtotal + shippingFee + appFee - discount;
        ...
        <section className="bg-white p-6 rounded-[32px] shadow-soft border border-surface-muted">
          ...
          {selectedAddress ? (
            <div className="flex flex-col gap-1">
              <div className="flex items-center gap-2">
                <span className="text-[14px] font-bold text-text-main">{selectedAddress.name}</span>
                <span className="w-1 h-1 bg-text-sub rounded-full" />
                <span className="text-[13px] text-text-sub">{selectedAddress.phone}</span>
              </div>
              <p className="text-[13px] text-text-sub leading-relaxed">{selectedAddress.details}</p>
            </div>
          ) : (
            <div 
              onClick={() => router.push("/profile/address")}
            ...
            >
              ...
              <span className="text-[12px] font-bold text-primary">Tambah Alamat Baru</span>
            </div>
          )}
        </section>
            ...
            {SHIPPING_METHODS.map((method) => (
              ...
              >
                <div className="flex flex-col">
                  <span className={cn("text-[14px] font-bold", selectedShipping.id === method.id ? "text-primary" : "text-text-main")}>
                    {method.name}
                  </span>
        ...
        <section className="flex flex-col gap-4">
          ...
          <div className="bg-white p-5 rounded-[32px] shadow-soft border border-surface-muted flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <span className="text-[14px] font-bold text-text-main">{selectedPayment.name}</span>
            </div>
      ...
      <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[440px] bg-white border-t border-surface-muted p-6 z-50 flex flex-col gap-6 shadow-2xl">
        <div className="flex flex-col gap-2">
          ...
          <div className="flex justify-between items-center text-[13px]">
            ...
            <span className="text-text-main font-bold">{formatPrice(shippingFee)}</span>
          </div>
          <div className="flex justify-between items-center text-[13px]">
